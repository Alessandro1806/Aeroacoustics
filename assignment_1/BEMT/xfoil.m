...
if (fid<=0),
  ...
  elseif ismac
      cmd = sprintf('cd %s && wine xfoil.exe < xfoil.inp > xfoil.out',['"',wd,'"']);
  else
...
if only <= 1% clear files for default run
  ...
  if length(pol.alpha) ~= Nalpha && ~isempty(pol.alpha)  % Check if xfoil failed to converge 
     warning('One or more alpha values failed to converge. Last converged was alpha = %f. Rerun with ''oper iter ##'' command.\n', pol.alpha(end))
  elseif isempty(pol.alpha)
      warning('No value of alpha converged. Rerun with ''oper iter ##'' command.\n')
  end

  delete('xfoil.inp')
  delete('xfoil.out')
  if exist(':00.bl','file'), delete(':00.bl'); end
