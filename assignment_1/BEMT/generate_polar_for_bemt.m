...
N_panels = 250;                         % Number of panels
iterations = 10000;                       % Number of iterations
N_crit = 9;                             % N_crit (transition)
xTRU = 0.01;                               % x of transition on the upper airfoil side (1 = free transition)
xTRL = 0.01;                               % x of transition on the lower airfoil side (1 = free transition)
xTRflag = (xTRU ~= 1) && (xTRL ~= 1);   % Flag to check if transition is forced on upper OR lower side of the airfoil
vacc = 0;                               % Viscous acceleration (higher -> more speed, but less stability)
